...
requirejs(['exports', './constants-5f55962b'], function (exports, constants) { 'use strict';
  ...
  class CompositeFn {
      constructor(fns) {
          ...
          this.fn = (x, y = new Float32Array(this.d)) => {
              ...
              constants.assert.equal(x.length, domain);
              constants.assert.equal(y.length, d);
              this.x.set(x);
          ...
          constants.assert(fns.length, 'fns cannot be empty');
  ...
  CompositeFn.apply = (fns, x, y) => {
      constants.assert.equal(x.length, y.length);
      for (const f of fns) {
          f.fn(x.subarray(0, f.domain), y.subarray(0, f.d));
          for (let i = 0; i < y.length; i++) {
              if (!isFinite(y[i]))
                  y[i] = Math.sign(y[i]) * constants.inf;
              if (isNaN(y[i]))
                  y[i] = 0;
          }
          x.set(y);
  ...
  class Sphere {
      constructor(d, r) {
          ...
          this.fn = (phi, y = new Float32Array(this.d)) => {
              const { d, r } = this;
              constants.assert.equal(phi.length, d - 1);
              constants.assert.equal(y.length, d);
              y[0] = r;
  ...
  class FuckedUpTorus {
      constructor(d, r, t) {
          ...
          this.fn = (phi, y = new Float32Array(this.d)) => {
              ...
              constants.assert.equal(phi.length, domain);
              constants.assert.equal(y.length, d);
              sphere.fn(phi, y);
  ...
  class Stereo {
      constructor(from, to) {
          ...
          this.fn = (x, y = new Float32Array(this.to)) => {
              ...
              constants.assert.equal(x.length, from);
              constants.assert.equal(y.length, to);
              if (from === to) {
                  y.set(x);
                  return y;
              }
  ...
  Stereo.up = (x, temp) => {
      constants.assert.equal(temp.length, x.length + 1);
      const n2 = norm2(x);
      const divisor = n2 + 1;
      temp[0] = (n2 - 1) / divisor;
  ...
  class Torus {
      constructor(d, r) {
          ...
          constants.assert(d > 2, `torus: expected d = ${d} > 2`);
          constants.assert(r.length == this.domain, `torus: expected r.length = ${r.length} == ${this.domain}`);
      }
      get domain() {
          return this.d - 1;
      }
  ...
  class Evaluator {
      constructor(scope, ast, hsv, chunk) {
          ...
          constants.assert(offset >= 0, `offset must be non-negative; got ${offset}`);
          constants.assert(limit <= n, `offset + size must be <= n; got ${offset} + ${size} = ${limit} > ${n}`);
          this.n = n;
          this.fn = fn;
          this.offset = offset;
...
//# sourceMappingURL=worker-53e248b7.js.map
